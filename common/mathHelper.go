...
type MathHelper interface {
	CastRay(origin *Point2D, worldMap world.WorldMap, rayAngle, visibility float64) *Point2D
	GetWorldElementProjection(
		playerPosition *Point2D,
		viewAngle float64,
		fov float64,
		worlElementPosition *Point2D,
...
func NewMathHelper(rayCaster RayCaster) (MathHelper, error) {
	if rayCaster == nil{
...
func (mathHelper *MathHelperImpl) CastRay(origin *Point2D, worldMap world.WorldMap, angle, maxDistance float64) *Point2D {
...
func (mathHelper *MathHelperImpl) GetWorldElementProjection(
	...
	worldElementVectorFromPlayer := &Point2D{
