...
//PlayableCharacterConfiguration provides fields to configure a PlayableCharacter.
type PlayableCharacterConfiguration struct {
	Velocity  float64
	StepAngle float64
}

//NewPlayableCharacter builds a new player from ithe input parameters.
func NewPlayableCharacter(initialPosition *common.Point2D, initialAngle float64, configuration *PlayableCharacterConfiguration, world WorldMap) Character {
	return &PlayableCharacter{
		pos:           initialPosition,
		angle:         initialAngle,
		velocity:      configuration.Velocity,
		world:         world,
		stepAngle:     configuration.StepAngle,
